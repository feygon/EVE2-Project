...
// Note: must be called with non-object-property callback function for complete. 
callbacks.session.setSession =
function setSession(req, res, playerID, mysql, complete) {
	...
	/**
	 * Callback function called when subcomplete event occurs.
	 * Increments the subcompleteCount and sets session variables if the count is greater than or equal to 1.
	 * @returns {void}
	 */
			...
			if (doneCount >= 1) {
				if (!req.session.shipNest) { // if nothing returned here.
					console.log("callbacks.post.player says: \n" 
						+ "Session set. Redirecting to /out_in_space/"
						+ "\n-------------end post.player----------------");
					res.redirect('/eve2/out_in_space/');
				} else {
					console.log("callbacks.post.player says: \n" 
						+ "Session set. Redirecting to /space_station/"
						+ "\n--------------end post.player---------------");
