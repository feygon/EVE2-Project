...
(function() {
    ...
    function initSidebarSliders(data) {
        var paramInit = {
            ...
            'snt_seed':              data.snt_seed,
            'lifestyle':             data.lifestyle_annual,
            'lifestyle_floor_monthly': data.lifestyle_floor_monthly,
            'charitable_monthly':    data.charitable_monthly,
            ...
            'mc_maintenance_monthly': data.mc_maintenance_monthly,
            'mc_residual_monthly':   data.mc_residual_monthly,
        ...
        // Optimizer toggle
        if (data.optimizer_enabled !== undefined) {
            $('#sidebar-optimizer').prop('checked', data.optimizer_enabled === 1);
        }
    }
    ...
    function updateDisplay($slider, param, value) {
        ...
        } else if (param === 'rental_income' || param === 'medical_base_monthly' || param === 'roommate_monthly' || param === 'ssdi_monthly' || param === 'lifestyle_floor_monthly' || param === 'charitable_monthly' || param === 'mc_maintenance_monthly' || param === 'mc_residual_monthly') {
            text = '$' + fmtNum(value) + '/mo';
        } else if (param === 'sdira_start' || param === 'managed_ira_start' || param === 'snt_seed' || param === 'total_mortgage_amount' || param === 'primary_house_value' || param === 'condo_value') {
            text = '$' + fmtNum(value);
    ...
    if (window.currentScenarioId) {
        ...
        // Optimizer toggle
        $('#sidebar-optimizer').on('change', function() {
            var checked = this.checked ? 1 : 0;
            updateAndReload('optimizer_enabled', checked);
        });
