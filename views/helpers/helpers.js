...
exports.lt = function(lvalue, rvalue) {
    return Number(lvalue) < Number(rvalue);
};

exports.gt = function(lvalue, rvalue) {
    return Number(lvalue) > Number(rvalue);
};
...
Condo (Arbor Roses): $${formatNum(projection.condo_value)} (+$${formatNum(projection.condo_appreciation)})`;
    ...
    if (projection.condo_sold) {
        ...
        if (projection.capital_gains && projection.capital_gains.totalTax > 0) {
            const cg = projection.capital_gains;
            tooltip += `\n\n  Capital Gains Tax:`;
            tooltip += `\n    Gain: $${formatNum(cg.gain)} (sale - cost basis)`;
            tooltip += `\n    Federal LTCG (15%): $${formatNum(cg.federalTax)}`;
            tooltip += `\n    Oregon (9.9%): $${formatNum(cg.oregonTax)}`;
            if (cg.niit > 0) tooltip += `\n    NIIT (3.8%): $${formatNum(cg.niit)}`;
            tooltip += `\n    Total CG Tax: $${formatNum(cg.totalTax)} (${(cg.effectiveRate * 100).toFixed(1)}%)`;
        } else if (projection.capital_gains && projection.capital_gains.totalTax === 0) {
            tooltip += `\n  Capital Gains Tax: $0 (stepped-up basis via LPOA)`;
        }
    }
...
exports.formatExpensesTooltip = function(expenses, projection) {
    ...
    // Debt
    // SNT condo P&I is paid from Person A's personal budget pre-Medicaid
    // (see _calculateYearExpenses.js:245-252 — added to fixedExpenses).
    // The marker `projection.snt_condo_mortgage` is set ONLY when the SNT
    // itself paid it (post-Medicaid); when unset, Person A paid.
    const sntPaidByPersonA = !projection || !projection.snt_condo_mortgage;
    const sntCondoPI = sntPaidByPersonA
        ? (expenses.SNT_condo_mortgage_interest || 0) + (expenses.SNT_condo_mortgage_principal || 0)
        : 0;
    const hasDebt = expenses.heloc_interest || sntCondoPI > 0;
    if (hasDebt) {
        lines.push('  Debt:');
        if (sntCondoPI > 0) {
            lines.push(`    SNT Condo Mortgage (P&I): $${formatNum(sntCondoPI)}`);
            personATotal += sntCondoPI;
        }
    ...
    // ═══ MAPT TRUST EXPENSES ═══
    // SSoT Theme 5: reads from mapt_ledger_entries (the same source as
    // formatMaptTooltip) so both tooltips are guaranteed consistent.
    // Previously this block inline-summed individual expense scalars —
    // a drift vector if the ledger gained or lost entries.
    if (isMapt) {
        lines.push('');
        lines.push('MAPT Trust Expenses:');
        lines.push('');

        const entries = projection.mapt_ledger_entries || [];
        ...
        for (const [category, catEntries] of byCategory) {
            const hasExpense = catEntries.some(e => e.side === 'expense');
            if (!hasExpense) continue;
            lines.push(`  ${category}:`);
            for (const e of catEntries) {
                if (e.side === 'expense') {
                    lines.push(`    ${e.label}: $${formatNum(Math.abs(e.amount))}`);
                } else if (e.side === 'note') {
                    lines.push(`    (${e.note || e.label})`);
                }
            }
        }

        const trustTotal = entries
        ...
        const hasMortgage = expenses.primary_mortgage_interest || expenses.SNT_condo_mortgage_interest || expenses.SNT_condo_mortgage_principal;
        if (hasMortgage) {
            ...
            const condoMort = (expenses.SNT_condo_mortgage_interest || 0) + (expenses.SNT_condo_mortgage_principal || 0);
            if (condoMort > 0) { lines.push(`    SNT Condo (P&I): $${formatNum(condoMort)}`); nonMaptExtra += condoMort; }
        }

        const hasCondo = expenses.AR_condo_property_tax || expenses.AR_condo_HOA || expenses.AR_condo_insurance || expenses.AR_condo_maintenance || expenses.AR_management_fee;
        if (hasCondo) {
            lines.push('  Condo:');
            if (expenses.AR_condo_property_tax) { lines.push(`    Property Tax: $${formatNum(expenses.AR_condo_property_tax)}`); nonMaptExtra += expenses.AR_condo_property_tax; }
            if (expenses.AR_condo_HOA) { lines.push(`    HOA: $${formatNum(expenses.AR_condo_HOA)}`); nonMaptExtra += expenses.AR_condo_HOA; }
            if (expenses.AR_condo_insurance) { lines.push(`    Insurance: $${formatNum(expenses.AR_condo_insurance)}`); nonMaptExtra += expenses.AR_condo_insurance; }
            if (expenses.AR_condo_maintenance) { lines.push(`    Maintenance: $${formatNum(expenses.AR_condo_maintenance)}`); nonMaptExtra += expenses.AR_condo_maintenance; }
            if (expenses.AR_management_fee) { lines.push(`    Management Fee: $${formatNum(expenses.AR_management_fee)}`); nonMaptExtra += expenses.AR_management_fee; }
        }
...
exports.formatDeductionsTooltip = function(projection) {
    ...
    if (breakdown.mortgage_interest > 0) {
        ...
        if (breakdown.SNT_condo_mortgage_interest > 0) {
            lines.push(`    SNT Condo (P&I): $${formatNum(breakdown.SNT_condo_mortgage_interest)}`);
        }
    }
...
// Format MAPT Net tooltip — consumes projection.mapt_ledger_entries as the
// SINGLE source of truth. Math and display are one concern.
// See: docs/ledger-refactor/PRD-ledger-refactor.md §5.1, §6.5
exports.formatMaptTooltip = function(projection) {
    ...
    // Group entries by category while preserving order of first appearance.
    const byCategory = new Map();
    for (const e of entries) {
        if (!byCategory.has(e.category)) byCategory.set(e.category, []);
        byCategory.get(e.category).push(e);
    ...
    for (const e of entries) {
        if (e.side === 'income') {
            lines.push(`${e.category} — ${e.label}: $${formatNum(e.amount)}`);
            incomeShown = true;
        }
    }
    if (!incomeShown) lines.push('(no trust income)');

    // --- Expenses & notes, grouped by category ---
    lines.push('');
    lines.push('=== MAPT Trust Expenses ===');
    for (const [category, catEntries] of byCategory) {
        const hasLine = catEntries.some(e => e.side !== 'income');
        if (!hasLine) continue;
        lines.push(`--- ${category} ---`);
        for (const e of catEntries) {
            if (e.side === 'expense') {
                lines.push(`  ${e.label}: -$${formatNum(-e.amount)}`);
            } else if (e.side === 'note') {
                // Zero-amount explanatory line — invariant-enforced.
                lines.push(`  (${e.note || e.label})`);
            }
        }
    }

    // Total derived from the SAME list rendered above (REQ-TT-02, Bug5 fix).
    const totalExpenses = entries
        .filter(e => e.side === 'expense')
        .reduce((s, e) => s + Math.abs(e.amount), 0);
    lines.push(`Total Trust Expenses: -$${formatNum(totalExpenses)}`);
...
// Strip "Sc.A — " or "Scenario A: " prefix from scenario names for nav display
exports.navLabel = function(name) {
